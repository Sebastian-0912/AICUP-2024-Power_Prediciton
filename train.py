...
def train_model(model, dataloader, location_id, num_epochs=5, learning_rate=1e-4, model_pth=False):
    ...
    for epoch in range(1, num_epochs + 1):
        total_loss = 0
        progress_bar = tqdm(dataloader, desc=f"L{location_id} | Epoch {epoch+2000}/{num_epochs+2000}")
        ...
        if epoch % 5000 == 0:
            checkpoint_path = os.path.join(checkpoint_dir, f"location_{location_id}_epoch_{epoch+2000}.pth")
            torch.save(model.state_dict(), checkpoint_path)
            print(f"Checkpoint saved for Location {location_id} at epoch {epoch+2000}.")
